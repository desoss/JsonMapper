...
public class JsonMapper {
	...
	public static InstanceDataMultiProvider ConvertJson(InstanceData input){
		...
		if(input.getMapTypeVMs() != null && input.getMapTypeVMs().isPresent() && !input.getMapTypeVMs().get().isEmpty()){
			output.setMapPublicCloudParameters(retrieveMapPublicCloudParameters(input.getMapTypeVMs(), input.getProvider()));
		}
		if(input.getMapVMConfigurations() != null && input.getMapVMConfigurations().isPresent() && !input.getMapVMConfigurations().get().getMapVMConfigurations().isEmpty() && input.getMapVMConfigurations().get().getMapVMConfigurations()!=null && input.getMapVMConfigurations().get().validate()){
			output.setMapVMConfigurations(retrieveMapVMConfigurations(input.getMapVMConfigurations()));
		}
		if(input.getPrivateCloudParameters() != null && input.getPrivateCloudParameters().isPresent() && input.getPrivateCloudParameters().get().validate()){
	...
	private static PublicCloudParametersMap retrieveMapPublicCloudParameters(Optional<Map<String, List<TypeVM>>> mapTypeVMsO, String provider){
	...
	private static PrivateCloudParameters retrievePrivateCloudParameters(Optional<PrivateCloudParameters> privateCloudParameters){
